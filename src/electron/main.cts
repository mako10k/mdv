...
const draftWorkspaceController = createDraftWorkspaceController({
  fs,
  fsPromises,
  path,
  randomUUID,
...
const trackedFileController = createTrackedFileController({
  fs,
  writeLog: (...parts) => writeLog(...parts),
  ensureEditorRuntimeState,
})
...
} = trackedFileController

const semanticCacheController = createSemanticCacheController({
  semanticCachePath,
  fs,
  writeLog: (...parts) => writeLog(...parts),
})
const {
  ...
  getSemanticRuntime,
  setSemanticRuntime,
  getAllEmbeddingCacheEntries,
  getEmbeddingCacheTotalBytes,
  getSemanticRuntimeBySourceKeyMap,
  buildEmbeddingCacheEntry,
} = semanticCacheController

const protectedContextController = createProtectedContextController({
  ensureEditorRuntimeState,
  estimateTokenCount,
...
async function ensureEmbeddingCacheEntries(texts, model = DEFAULT_EMBEDDING_MODEL) {
  ...
  const missingTexts = uniqueTexts.filter((text) => {
    ...
    const existingEntry = getEmbeddingCacheEntry(cacheKey)
...
async function ensureSemanticRuntimeForTarget(editorWindow, payload) {
  ...
  const cachedRuntime = getSemanticRuntime(sourceKey)
  ...
  const runtime = {
    ...
    chunks: chunks.map((chunk) => ({
      ...chunk,
      embedding: getEmbeddingCacheEntry(chunk.cacheKey)?.embedding || null,
    })).filter((chunk) => Array.isArray(chunk.embedding)),
  }

  setSemanticRuntime(sourceKey, runtime)
  markEmbeddingUsageCountsDirty()
  pruneSemanticRuntimes()
...
async function semanticSearchForWindow(editorWindow, payload) {
  ...
  const queryEmbedding = getEmbeddingCacheEntry(buildEmbeddingCacheKey(DEFAULT_EMBEDDING_MODEL, query))?.embedding
