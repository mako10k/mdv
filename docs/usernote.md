...
- [ ] 25. Editor領域のPadding(もしくは内部コンテナのMargin)を減らす
  Editor領域の書き始め位置から上と左の余白が広すぎるため、これを減らしてより多くの情報を表示できるようにする。特に左の余白は、アウトラインや行番号と合わせて全体の見える情報量に影響するため、適切なバランスを検討する。