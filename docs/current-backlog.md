...
- P2 Editor Expansion のうち MD-BL-020 は棚卸済みで、`write_target dryRun` preview foundation に続き、live active editor の interactive change proposal と hunk apply / discard まで完了した。model response から terminal resolution までの proposal 専用 Electron integration regression も validation gate として追加済みである。`backlog_state: accepted_active` は維持し、次の最小範囲では proposal hunk の manual text editing と再検証 / canonicalization contract を扱う
...
- MD-BL-020 は save conflict preview だけでなく AI 書き込みの merge UI foundation として扱う。active-editor proposal の hunk apply / discard は完了し、manual hunk text editing は残 scope とする
...
2026-07-13 MD-BL-020 interactive change proposal slice 結果:

- MD-BL-020 は `inventory_status: inventory_confirmed`、`backlog_state: accepted_active`、`contract_state: active_contract` を維持する。この slice では model tool loop の eligible な live active-editor `write_target(dryRun: true)` を main-owned transient proposal とし、renderer で canonical line hunk ごとの apply / discard を選んで final Apply または Cancel できるようにした
- design_contract は [docs/ai-chat-design.md](ai-chat-design.md) の Interactive Active-Editor Change Proposal、decision record は [ADR 0023](adr/0023-main-owned-interactive-change-proposal-lifecycle.md) とする。proposal は document instance / path / exact baseline と before / after Markdown を main process で束縛し、作成時に model turn を sibling tool / 次 model iteration より前に停止する。Apply は selected hunk IDs だけを受け、main-derived result を renderer が同じ request handler 内で identity / baseline 再検証してから適用する
- lifecycle は `pending -> applying -> applied | stale | indeterminate` または `pending -> cancelled | invalidated` の single-use とし、Apply dispatch 後の window close を含む indeterminate outcome は自動 retry しない。proposal は editor ごとに 1 件、TTL 10 分、before + after raw payload 合計 2 MiB を上限とし、超過時は通常の bounded / temp-buffer dry-run preview へ戻す。full proposal content / authoritative hunks は OpenAI input、chat transcript、generic tool-event JSON、log に流さず、dedicated typed IPC だけで扱う
- allowed_scope は live active-editor dry-run proposal、candidate-first ordering、detail loading 中から背面操作を止める keyboard-modal review、line hunk の apply / discard selection、zero-selection Apply guard、final Apply / Cancel、document identity / baseline の atomic stale check、terminal resolution metadata に限る。detail load failure は fallback Cancel の terminal resolution を確認できるまで modal と active ID を維持し、renderer loss は main-owned proposal を terminal 化して、local UI だけ失って TTL まで残す状態を作らない。blocked_scope は non-dry direct write の挙動変更、`:new` / temp buffer / structure mutation proposal、save-conflict flow の再実装、snapshot restore、global suggest mode、manual hunk text editing である
- 根拠 anchor は `src/electron/main/change-proposal-controller.cts` の bounded single-use proposal registry、`src/electron/main.cts` の dry-run tool-loop termination / atomic apply orchestration、`src/electron/main/main-ipc.cts` と `electron/preload.cjs` の dedicated typed IPC、`src/shims.d.ts` の proposal contract、`src/change-preview/ChangePreviewDialog.tsx` と `src/App.tsx` の hunk selection / apply / cancel UI、`src/ai-chat/ChatApp.tsx` の proposal-pending / resolution handling である。自動テストの証拠は、controller / candidate ordering / public-display sanitization / IPC delegation の Node tests、desktop bridge を mock した loading modal / hunk selection / stale / cancel の browser tests、`tests/e2e-electron/ai-change-proposal.spec.ts` の proposal 専用 Electron integration regression に分かれている。Electron integration regression は外部 Responses API だけを loopback SSE に置き換え、実 OpenAI SDK から main candidate-first ordering、renderer capture、dedicated preload open / detail IPC、selected-hunk Apply acknowledgement、terminal resolution までを一続きに通す。先行 non-dry sibling の非実行、provisional / raw proposal の transcript・log 非漏洩、Apply 後も model request が 1 回のままであること、実 bridge write による baseline drift が `stale` / `baseline-changed` で fail-closed になることも固定する
- manual hunk text editing は未実装のため MD-BL-020 全体を completed にはしない。実 IPC chain の proposal 専用 Electron integration validation gate は完了済みである。次 slice では edited hunk body の ownership、line-coordinate canonicalization、baseline revalidation、invalid edit rejection を先に contract 化して、単なる renderer-side replacement text の受け渡しに広げない
...
- MD-BL-020 は一部完了・後続あり。MD-BL-016 の save conflict merge preview は完了済みとして再実装せず、first slice の AI `write_target dryRun` preview foundation に続き、live active editor の main-owned interactive proposal、canonical line hunk apply / discard、atomic stale check を追加した。manual hunk text editing は残 scope である。
