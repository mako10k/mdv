...
- P2 Editor Expansion のうち MD-BL-020 は棚卸済みで、`write_target dryRun` preview foundation に続き、live active editor の interactive change proposal と hunk apply / discard まで完了した。`backlog_state: accepted_active` は維持する。次の最小範囲では、最初に model response から terminal resolution までの proposal 専用 Electron integration regression を validation gate として追加し、その後 proposal hunk の manual text editing と再検証 / canonicalization contract を扱う
...
- 根拠 anchor は `src/electron/main/change-proposal-controller.cts` の bounded single-use proposal registry、`src/electron/main.cts` の dry-run tool-loop termination / atomic apply orchestration、`src/electron/main/main-ipc.cts` と `electron/preload.cjs` の dedicated typed IPC、`src/shims.d.ts` の proposal contract、`src/change-preview/ChangePreviewDialog.tsx` と `src/App.tsx` の hunk selection / apply / cancel UI、`src/ai-chat/ChatApp.tsx` の proposal-pending / resolution handling である。自動テストの証拠は、controller / candidate ordering / public-display sanitization / IPC delegation の Node tests と、desktop bridge を mock した loading modal / hunk selection / stale / cancel の browser tests に分かれている。既存 Electron E2E 全件は周辺回帰を確認するが、model response から main ordering、preload event、modal、apply acknowledgement、terminal resolution までを実 IPC で一続きに通す proposal 専用 Electron integration test ではない。この slice の coverage をその end-to-end chain の実証とは扱わない
- manual hunk text editing は未実装のため MD-BL-020 全体を completed にはしない。次 slice の着手前に実 IPC chain の proposal 専用 Electron integration regression を追加し、その後 edited hunk body の ownership、line-coordinate canonicalization、baseline revalidation、invalid edit rejection を先に contract 化して、単なる renderer-side replacement text の受け渡しに広げない
