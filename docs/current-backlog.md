...
- `backlog_state: completed`
- `inventory_status: inventory_confirmed`
- `user_outcome`: preview / AI chat の Mermaid 図をクリックして専用 window で開き、本文 layout を変えずに画面幅・高さを超える大きさまで拡大でき、拡大領域全体を scroll、drag pan、reset できる
...
- `evidence`: `src/mermaid-viewer/`、`src/electron/main/window-controller.cts`、`src/electron/main/main-ipc.cts`、初回payloadをrenderer購読まで保持する`electron/preload.cjs`、`tests/e2e/app-layout.spec.ts`、`tests/node/electron-main-ipc.spec.mjs`、`tests/node/electron-main-window-controller.spec.mjs`、multi-entry release fingerprint / packaged security checks。initial 100% と 400% の scroll extent 増加、両軸の scroll end 到達を focused browser regression で検証し、`npm run lint`、`npm run build`、focused Mermaid viewer tests（2 pass）を通した。既存 pointer drag pan は browser test、owner close / navigation denial は Node test の過去 evidence に留まり、この修正 turn では再実行していない。assistant activation、実Electron E2E、Windows packaged smokeもこの修正 turnでは未実行であり、実施済みとは扱わない
...
2. [棚卸確認済み / current accepted scope 完了] MD-BL-020 変更プレビューと merge UI 基盤
...
- MD-BL-025:
  ...
  - `design_contract`: [Typography Interaction Design](typography-interaction-design.md)
  - `contract_state: active_contract`
...
- first slice 後の scope は renderer-side の reusable change preview / merge UI surface と hunk 単位 apply / discard / edit とした。AI snapshot restore の preview / resolve action は downstream consumer 候補だが、AI-ED 側の棚卸前に MD-BL-020 accepted scope として先取りせず、MD-BL-016 の save conflict polish とも混同しない

2026-07-13 MD-BL-020 interactive change proposal / manual edit slices 結果:

- MD-BL-020 は `inventory_status: inventory_confirmed`、`backlog_state: completed`、`contract_state: active_contract` とする。model tool loop の eligible な live active-editor `write_target(dryRun: true)` を main-owned transient proposal とし、renderer で canonical line hunk ごとの apply / discard / manual edit を選んで final Apply または Cancel できるようにした
- design_contract は [docs/ai-chat-design.md](ai-chat-design.md) の Interactive Active-Editor Change Proposal、decision record は [ADR 0023](adr/0023-main-owned-interactive-change-proposal-lifecycle.md) とする。proposal は document instance / path / exact baseline と stable hunk envelope を main process で束縛し、作成時に model turn を sibling tool / 次 model iteration より前に停止する。manual edit は dedicated revise-hunk IPC で revision / proposal fingerprint を CAS し、main が LF / CRLF、line boundary、no-op、raw size を再検証して canonical candidate と preview diff を atomic 更新する。Apply は proposal ID、current revision / fingerprint、selected hunk IDs だけを受け、main-derived result を renderer が同じ request handler 内で identity / baseline 再検証してから適用する
- lifecycle は edit の `pending@r -> pending@(r+1)` と、`pending -> applying -> applied | stale | indeterminate` または `pending -> cancelled | invalidated` の single-use terminal transition とし、Apply dispatch 後の window close を含む indeterminate outcome は自動 retry しない。proposal は editor ごとに 1 件、TTL 10 分、baseline + current all-hunks candidate raw payload 合計 2 MiB を上限とし、作成時と edit 時に検証する。full proposal content / authoritative hunks / manual edit body は OpenAI input、chat transcript、generic tool-event JSON、terminal resolution metadata、log に流さず、dedicated typed IPC だけで扱う
- allowed_scope は live active-editor dry-run proposal、candidate-first ordering、detail loading 中から背面操作を止める keyboard-modal review、line hunk の apply / discard selection、stable envelope 内の manual edit、zero-selection / unsaved-draft Apply guard、final Apply / Cancel、document identity / baseline の atomic stale check、terminal resolution metadata に限る。detail load failure は fallback Cancel の terminal resolution を確認できるまで modal と active ID を維持し、renderer loss は main-owned proposal を terminal 化して、local UI だけ失って TTL まで残す状態を作らない。blocked_scope は non-dry direct write の挙動変更、`:new` / temp buffer / structure mutation proposal、save-conflict flow の再実装、snapshot restore、global suggest mode、original hunk envelope を越える全文 editing である
- 根拠 anchor は `src/electron/main/change-proposal-controller.cts` の bounded proposal registry / stable envelope composition / revision CAS、`src/electron/main.cts` の dry-run tool-loop termination / revise-hunk / atomic apply orchestration、`src/electron/main/main-ipc.cts` と `electron/preload.cjs` の dedicated typed IPC、`src/shims.d.ts` の proposal contract、`src/change-preview/ChangePreviewDialog.tsx` と `src/App.tsx` の hunk selection / manual edit / apply / cancel UI、`src/ai-chat/ChatApp.tsx` の proposal-pending / resolution handling である。自動テストの証拠は、controller の ownership / canonical LF / CRLF / EOF / stale revision / size / literal / partial-apply tests、candidate ordering / public-display sanitization / IPC delegation の Node tests、desktop bridge を mock した loading modal / natural textarea line-boundary canonicalization / hunk edit rejection・retry / acknowledgement-loss detail recovery / focus / stale / cancel の browser tests、`tests/e2e-electron/ai-change-proposal.spec.ts` の proposal 専用 Electron integration regression に分かれている。Electron regression は外部 Responses API だけを loopback SSE に置き換え、実 OpenAI SDK から main candidate-first ordering、renderer capture、dedicated preload open / detail / revise-hunk IPC、selected-hunk Apply acknowledgement、terminal resolution までを一続きに通す。先行 non-dry sibling の非実行、provisional / raw proposal / manual edit body の transcript・generic event・request・resolution・log 非漏洩、Apply 後も model request が 1 回のままであること、manual edit 後の実 bridge write による baseline drift が `stale` / `baseline-changed` で fail-closed になることも固定する
- current accepted scope は完了した。MD-BL-020 の downstream consumer 拡張や envelope 外編集を暗黙に追加せず、次の P2 候補 MD-BL-021 は design / inventory / user acceptance を別に確認する
...
- ENG-BL-003:
  ...
  - `accepted_scope`: dependency / cross-boundary / packaging 変更では、full regression と candidate 生成より前に、実行経路、生成 bundle、package audit の証明範囲、fail-closed 回帰を対象とする early contract review を置く。candidate 後の exact-diff consistency / plain-eye / packaging review は最終 gate として維持し、early review の代替にはしない
  - `result`: release workflow を early contract review と final exact-diff review の 2 checkpoint に分離した。`npm run codex:validate` は dependency / generated-runtime wiring、cross-process contract、packaging / distribution の diff で early review の対象 area、review agent、actual runtime / generated artifact evidence focus を表示し、committed WIP は explicit base / head range で対象を固定できる。early blocker、final defer、最大 review 回数、pass 無効化条件、記録先を明文化した。WIP closeout は release gate 完了から分離し、重い処理停止、未完 gate と再開点の記録、mandatory review、WIP commit / push だけで閉じる
  - `evidence`: `docs/release-workflow.md`、`AGENTS.md`、`DEVELOPMENT.md`、ADR 0014、`scripts/codex-workspace.mjs`、`tests/release/codex-workspace-early-review.spec.mjs`
...
- ENG-BL-004:
  ...
  - `inventory_evidence`: [ENG-BL-004 Plugin Architecture Inventory](milestones/plugin-architecture-inventory.md)
...
- Plugin package が将来 SKILL metadata/resources を宣言しても、package provenance / availability は Plugin lifecycle、matching / enabled state / instruction loading / turn diagnostics は AI-CFG-002 が所有する。Plugin Manifest Catalog の受理だけでは Skill injection や script execution を受理しない
...
- MD-BL-020 current accepted scope は完了。MD-BL-016 の save conflict merge preview は完了済みとして再実装せず、AI `write_target dryRun` preview foundation、live active editor の main-owned interactive proposal、canonical line hunk apply / discard、stable envelope 内の manual edit、main-side revalidation、atomic stale check を一続きで固定した。
...
1. P2 Editor Expansion の棚卸へ戻る: MD-BL-021、MD-BL-014、MD-BL-008、MD-BL-009、MD-BL-010、MD-BL-011、MD-BL-025、MD-BL-026
2. Supporting Backlog の残件: ENG-BL-001、REL-BL-001、REL-BL-002 を棚卸する
