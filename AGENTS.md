...
- Use [docs/decision-governance.md](docs/decision-governance.md) before turning backlog text, ADR text, or user requests into implementation scope. Current design / contract docs are the implementation authority; ADRs record decisions and rationale; current-backlog controls accepted work and priority but does not override design contracts.
...
- Use [docs/image-storage-design.md](docs/image-storage-design.md) before changing image storage, relative image compatibility, asset workspace/materialization, or image export behavior.
...
- Update the ADR in the same turn as the code or doc change that depends on it. Do not use an ADR-only edit to redefine current implementation behavior; update the governing design / contract doc in the same turn.
