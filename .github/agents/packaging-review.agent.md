...
When the prompt explicitly requests an early packaging contract review:

- Review only the explicit base / head range or named packaging path.
- Judge whether candidate generation may begin, not commit, promotion, or release readiness.
- Treat a broken source-to-candidate path, stale-artifact false green, false primary evidence, or missing fail-closed targeted check as blockers.
- Do not require candidate hashes, packaged smoke, promoted metadata, or release assets before candidate generation.
...
- End with `early pass`, `early pass with deferred findings`, or `early blocked`; do not issue a release-readiness verdict.
